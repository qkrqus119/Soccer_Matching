...
function markerMove(marker, title, event) {
  ...
  searchDetailAddrFromCoords(clickedMarkerPosition, function(result, status) {
    if (status === kakao.maps.services.Status.OK) {
      ...
      if (detailAddr === '') {
        document.getElementById("addressLong").value = event.target.parentElement.childNodes[3].textContent + ' ' + event.target.parentElement.childNodes[1].textContent;
        document.getElementById("address").value = result[0].road_address.address_name;
        if(event.target.parentElement === "SPAN"){
        	document.getElementById("placeName").value = event.target.parentElement.parentElement.childNodes[0].textContent;
        }else{
        	document.getElementById("placeName").value = event.target.parentElement.childNodes[0].textContent;
        }
      } else {
        document.getElementById("addressLong").value = detailAddr;
        document.getElementById("address").value = result[0].road_address.address_name;
        if(event.target.parentElement === "SPAN"){
        	document.getElementById("placeName").value = event.target.parentElement.parentElement.childNodes[0].textContent;
        }else{
